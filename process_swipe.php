...
if ($action === 'like') {
    if ($existing_match) {
        ...
        $new_estado = 'empresa_gusta'; // Default if company likes

        if ($current_estado === 'estudiante_gusta') {
            $new_estado = 'match'; // Mutual like!
        } else if ($current_estado === 'rechazado') {
            // Company likes, overriding previous reject by company (or student)
            $new_estado = 'empresa_gusta';
        }
        // If it was already 'match' or 'empresa_gusta', keep it as 'empresa_gusta' or 'match'
        // 'empresa_gusta' remains 'empresa_gusta'
        // 'match' remains 'match'
        ...
        if ($stmt_update->execute()) {
            $response = ['success' => true, 'message' => ($new_estado === 'match' ? '¡Match confirmado!' : 'Interacción de empresa registrada.')];
        } else {
            $response = ['success' => false, 'message' => 'Error al actualizar interacción (like): ' . $mysqli->error];
        }

    } else {
        // No record, insert new one (Company initiates like)
        $new_estado = 'empresa_gusta';
        ...
        if ($stmt_insert->execute()) {
            $response = ['success' => true, 'message' => 'Interacción de empresa registrada.'];
        } else {
